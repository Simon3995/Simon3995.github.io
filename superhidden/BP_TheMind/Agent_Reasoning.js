class Agent {
    ...
    get_action() {
        ...
        // reasoning
        if (this.game.hand[0] - this.game.top_card == 1) return "play";     // if I have lowest card, play immediately
       
        if (this.game.hand[0] - this.game.top_card == 2 &&
            this.game.time_since_last_card >= 3) return "play";             // if my card is 2 away and others don't play immediately, play
        
        if (this.game.hand[0] - this.game.top_card == 3 &&
            this.game.time_since_last_card >= 5) return "play";             // if my card is 3 away and others don't play quickly, play
        
        if (this.game.hand[0] == Settings.decksize) {               // if I have the highest card
            if (cards_amt() > 1) {
                return time > this.shuriken_patience && this.game.shurikens ? "shuriken" : "wait";                                      // wait if other cards are still in play
            } else {
                return "play";                                      // play immediately if not
            }
        }
